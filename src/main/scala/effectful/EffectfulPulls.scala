...
//res7: Pull[Nothing, Int, Option[Unit]] = FlatMap(Uncons(FlatMap(FlatMap(Output(1),repl.MdocSession$MdocApp$$Lambda$19950/0x00000006037c0000@331d1236),repl.MdocSession$MdocApp$$Lambda$19951/0x00000006037c03d0@4eb1a0fd)),effectful.EffectfulPulls$Pull$$Lambda$19679/0x00000006037b4f48@30b09579)
    ...
    def step[F2[x] >: F[x], O2 >: O, R2 >: R](
      ...
      this match
        ...
        case Uncons(source) =>
          println(s"Uncons ${source}");source.step.map{s => println(s"hello inside uncons ${s} and ${s.asInstanceOf[R2]}");Left(s.asInstanceOf[R2])}
        case FlatMap(source, f) => 
          source match
            case FlatMap(s2, g) =>
             println(s"Nested FlatMap ${source}");
             val y=s2.flatMap{x => val c=g(x).flatMap(y => f(y)); println(s"This is f ${c}");c}// the println is only executed when the function to flatMap is called 
             
             y.step
            case other => other.step.flatMap:
              case Left(r) => println(s"other result ${r} and f is ${f} and it is ${f(r)}");f(r).step
    ...
    def fold[F2[x] >: F[x], R2 >: R, A](init: A)(f: (A, O) => A)(
      ...
      //flatMap on F[_]
      step.flatMap:
        case Left(r) => println(s"final result in fold is ${r} and value is ${init}");F.unit((r, init))
        case Right((hd, tl)) => println(s"Monadic flatMap value ${hd} and tail ${tl}");tl.fold(f(init, hd))(f)
    ...
    def toList[F2[x] >: F[x]: Monad, O2 >: O]: F2[List[O2]] =
      ...
      .map{x=>println(s"dude ${x}");x(1).result}
      //only happens after all the folding is complete
    ...
    def mapOutput[O2](f: O => O2): Pull[F, O2, R] =
      uncons.flatMap:
        case Left(r) => println("mapping");Result(r)
  ...
  object Pull:
    ...
    given pullGivenInstance[F[_], O]: Monad[[x] =>> Pull[F, O, x]] with
  ...
  object Stream:
    ...
    //def emit[F[_] >: Nothing1, O](o: O): Stream[F, O] = Pull.Output(o).toStream
    ...
    extension [F[_], O](self: Stream[F, O])
      ...
      def run2(using Monad[F]): F[Unit] =
        self.fold(())((_, _) => ()).map(_(1))
    ...
    // given streamGivenInstance[F[_]]: Monad[[x] =>> Stream[F, x]] with
    //   def unit[A](a: => A): Stream[F, A] = 
    //     println("stream unit");Pull.Output(a)
